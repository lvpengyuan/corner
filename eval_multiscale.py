...
from data import  train_cfg, cfg_512x512, cfg_768x768, cfg_768x1280, cfg_1280x1280, AnnotationTransform, ICDARDetection, detection_collate, TD500Detection, COCODetection, MLTDetection
...
def show_box(img, boxes, h, w):
    img = img[0].numpy().transpose(1,2,0) + np.array([122.67891434, 116.66876762, 104.00698793])
    img = img.astype(np.uint8)
    img = Image.fromarray(img).resize((w[0], h[0]))
    draw = ImageDraw.Draw(img)
    for box in boxes:
        #draw.polygon(box[:-1], outline=(255, 0, 0))
        draw.line([box[0], box[1], box[2], box[3]], width=4, fill=(0, 255, 0))
        draw.line([box[2], box[3], box[4], box[5]], width=4, fill=(0, 255, 0))
        draw.line([box[4], box[5], box[6], box[7]], width=4, fill=(0, 255, 0))
        draw.line([box[6], box[7], box[0], box[1]], width=4, fill=(0, 255, 0))
        
    return img

def eval_img(out, seg_pred, seg_map, rpsroi_pool, img):
    ...
    candidate_boxes = get_boxes(top_left_points, top_right_points, bottom_right_points, bottom_left_points, seg, seg_map, rpsroi_pool, 0.65)
...
def main():
    ...
    parser.add_argument('--resume', dest='resume',
                        ...
                        default='./weights/ic13_60.pth', type=str)
    parser.add_argument('--version', dest='version', help='512x512, 768x768, 768x1280, 1280x1280', default='768x768', type=str)
    parser.add_argument('--dataset', dest='dataset', help = 'ic15, ic13, td500, coco, mlt'
                        ,default='ic13', type=str)
    ...
    parser.add_argument('--out', dest='out',
                        ...
                        default='./outputs/imgs/ic13/', type=str)
    ...
    parser.add_argument('--mlt_root', default='/home/lpy/Datasets/MLT_test/', type=str, help='Location of data root direction')
    parser.add_argument('--vis', default=True, type=bool, help='Vis the bounding box')
    ...
    for cfg in cfgs:
        ...
        for i, sample in enumerate(data_loader, 0):
            ...
            candidate_boxes = eval_img(out, seg_pred, seg_map, rpsroi_pool,  img)
            temp_boxes = []
